class Game
  ...
  attr_reader :player, :interactables
  ...
  def handle_interaction args
    ...
    interactable = @interactables.find { |candidate| candidate.contains_point?(args.inputs.mouse.click) }
  ...
  def render_lit_scene args
    ...
    @interactables.each { |interactable| interactable.render(args, args.outputs[:scene]) }
    ...
    @interactables.each { |interactable| interactable.render_light(args, args.outputs[:darkness]) }
